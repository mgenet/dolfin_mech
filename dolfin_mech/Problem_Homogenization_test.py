...
def compute_cell_geometry(mesh, dim):
    X = mesh.coordinates()
    xmin, xmax = float(X[:, 0].min()), float(X[:, 0].max())
    ymin, ymax = float(X[:, 1].min()), float(X[:, 1].max())

    if dim == 2:
        # IMPORTANT: dmech.PeriodicSubDomain expects bbox as a flat array
        ...
        vol = (xmax - xmin) * (ymax - ymin)
        return bbox, vertices, vol

    elif dim == 3:
        zmin, zmax = float(X[:, 2].min()), float(X[:, 2].max())
        bbox = np.array([xmin, xmax, ymin, ymax, zmin, zmax], dtype=float)
        # vertices: if your PeriodicSubDomain needs 8 vertices, we can construct them
        vertices = None
        vol = (xmax - xmin) * (ymax - ymin) * (zmax - zmin)
        return bbox, vertices, vol
...
mesh_params = {
    ...
    "xshift": -0.5, "yshift": -0.5, "zshift": -0.5,
    "r0": 0.2, "l": 0.05,
...
dim = 2
mat_params = {"E": 3.142320556943062, "nu": 0.3088312124587496}

bbox, vertices, vol = compute_cell_geometry(mesh, dim)

# -----------------------------
# instantiate problem correctly
# -----------------------------
hp = HomogenizationProblem(
    dim=dim,
    mesh=mesh,
...
# run what you need
# -----------------------------
lambda_hom, mu_hom = hp.get_lambda_and_mu()
print("lambda_hom =", lambda_hom)
print("mu_hom     =", mu_hom)

kappa_tilde = hp.get_kappa()
