...
def run_PoroDisc_Coupled(
    ...
    # ------------------------- Mesh ------------------------- #
    # cube_params  = {"mesh_filebasename": "./mesh"}
    # mesh, boundaries_mf, xmin_id, xmax_id, ymin_id, ymax_id, zmin_id, zmax_id = dmech.run_RivlinCube_Mesh(dim=3, params=cube_params)

    # mesh = dolfin.Mesh()
    # with dolfin.XDMFFile("./mesh/3D/voronoi_3D_with_surfaces.xdmf") as infile:
    #     infile.read(mesh)
    mesh = dolfin.Mesh()
    with dolfin.XDMFFile("./mesh/3D/bar3D.xdmf") as infile:
        infile.read(mesh)

    # mesh = dolfin.Mesh()
    # with dolfin.XDMFFile("./mesh/3D/bar_with_pillars.xdmf") as infile:
    #     infile.read(mesh)
    ...
    r = 0.3  # adjust as needed

    inlet_center = [0.5, 0,0.5]
    outlet_center = [0.5, 1,0.5]

    # Mark cells near inlet
    class InletDomain(dolfin.SubDomain):
        def inside(self, x, on_boundary):
            return ((x[0] - inlet_center[0])**2 +
                    (x[1] - inlet_center[1])**2 +
                    (x[2] - inlet_center[2])**2) < r**2

    class OutletDomain(dolfin.SubDomain):
        def inside(self, x, on_boundary):
            return ((x[0] - outlet_center[0])**2 +
                    (x[1] - outlet_center[1])**2 +
                    (x[2] - outlet_center[2])**2) < r**2
    ...
    # inlet_vol = assemble(Constant(1.0) * dx(inlet_id, domain=mesh, subdomain_data=domains_mf))
    # outlet_vol = assemble(Constant(1.0) * dx(outlet_id, domain=mesh, subdomain_data=domains_mf))

    # print("Inlet domain volume:", inlet_vol)
    # print("Outlet domain volume:", outlet_vol)
    # print(aa
    # )
    ...
    bot_line = dolfin.CompiledSubDomain("near(x[1], y_bot, tol)", y_bot=y_min, tol=tol)

    z_max_surface = dolfin.CompiledSubDomain("near(x[2], z_top, tol)", z_top=z_max, tol=tol)
    z_min_surface = dolfin.CompiledSubDomain("near(x[2], z_top, tol)", z_top=z_min, tol=tol)
    ...
    val=[0.0,0.0,0.0],  # fully fixed in x and y
    method="pointwise"
    )
...
run_PoroDisc_Coupled(
    ...
    res_basename="results/3DPoroflow",
    verbose=0
)